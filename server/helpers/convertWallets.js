...
async function convertWallets(coin) {
  ...
  const { mintInfo } = coin;
  const { address, name, symbol, logouri, millionTimeStamp, athprice, timestamps } = mintInfo;
  ...
  const walletResults = await Promise.all(
    walletAddresses.map(async (walletAddress) => {
      ...
      const runners = walletGroups[walletAddress].map(transactions => {
        return { address, name, symbol, logouri, millionTimeStamp, transactions, athprice, timestamps };
      });
